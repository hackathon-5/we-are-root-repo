...
def list_repos():
    ...
    for repo_id in repos_watched:
        ...
        for issue in repo.get_issues():
            ...
            issue.sortable_timestamp = arrow.get(issue.updated_at).timestamp
            all_issues.append(issue)

        for comment in repo.get_issues_comments():
            comment.repo = repo.full_name
            comment.sortable_timestamp = arrow.get(comment.updated_at).timestamp
            all_comments.append(comment)

    all_issues.sort(key=lambda i: i.sortable_timestamp, reverse=True)
    all_comments.sort(key=lambda c: c.sortable_timestamp, reverse=True)
